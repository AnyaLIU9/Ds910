...
本文只处理 W8A8 转换、启动和压测，不测试 BF16 服务，也不说明源码包如何制作或上传。
...
源码包：/data/models/Tensor/prometheus-infer.zip
源码目录：/data/models/prometheus-infer
...
export SOURCE_ARCHIVE=/data/models/Tensor/prometheus-infer.zip
export SOURCE_DIR=/data/models/prometheus-infer
...
test ! -e "$SOURCE_DIR" || {
  echo "错误：$SOURCE_DIR 已存在，请人工确认，不自动覆盖。" >&2
  exit 1
}

python3 -m zipfile -e "$SOURCE_ARCHIVE" /data/models
...
如果实际文件名不同，只修改 `SOURCE_ARCHIVE`。解压后的 `/data/models/prometheus-infer` 必须直接包含 `python/`、`scripts/`、`tools/` 和 `requirements-ascend.txt`，不能再多套一层目录。
...
docker run --rm -it \
  ...
  -w /data/models/prometheus-infer \
  "$IMAGE" bash
```
...
```bash
cd /data/models/prometheus-infer
...
不要执行 `scripts/build-release-wheels.sh`，它包含 CUDA release/kernel-cache 构建逻辑。
...
应显示 910B2、单个可见设备、40 层、192 专家、top-k 8 和 `ascend_w8a8`。
...
若服务器没有整个 `Ds910` 目录，把该脚本放到 `/data/models/prometheus-infer/bench_concurrency.py` 并修改下方路径。
...
先在 `MAX_RUNNING_REQUESTS=1` 下测单流：
...
单流通过后，停止服务，把第 9 节的 `MAX_RUNNING_REQUESTS=1` 改为 `2` 并重新启动，再测：
...
当前 GDN、SDPA、专家拷贝/计算重叠还未完成 910B2 生产级优化，不要一开始使用并发 `5 10 20 30`。压测期间运行：
