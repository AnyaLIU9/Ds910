...
NPU: 2张空闲 Ascend 910B2，物理设备号 3、5
单卡 HBM: 64 GiB
```
...
先阅读 `ENVIRONMENT_CHECKLIST.md`，然后运行仓库脚本：
...
双实例前额外要求：建议至少384 GiB可用 DDR，最好512 GiB或更多。
...
两个实例总工作线程先控制在192，给系统和负载工具保留约64核；不要直接让两个实例各使用128线程占满全部256核。

最大聚合吞吐：分别向两个端口发送负载，并发50/50，输出 token/s求和。真实入口延迟：在两个端口前部署 `least_conn`负载均衡，对统一入口做并发100。

理想线性上限约26～32 token/s；DDR争抢后可能约18～29 token/s。该范围只是预估。计算：

```text
scale_efficiency = aggregate_TPS / (2 × single_instance_TPS)
```
